...
import { getServices, addServiceToSheet, updateServiceInSheet, deleteServiceFromSheet, WEB_APP_URL } from '../services/googleSheetService';
  ...
  const applyAIRanking = useCallback(async () => {
    ...
    try {
      ...
      if (servicesToUpdate.length > 0) {
         // O backend não suporta uma ação de atualização em massa, então enviamos as solicitações de atualização em paralelo.
        await Promise.all(servicesToUpdate.map(s => updateServiceInSheet(s)));
      }
