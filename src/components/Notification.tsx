...
const Notification: React.FC<NotificationProps> = ({ message, type }) => {
  ...
  return (
    ...
    >
      <div
        role="alert"
        className={`relative flex items-center p-4 pr-10 border-l-4 rounded-lg shadow-lg transition-all duration-500 transform ${typeClasses[type]} ${visible ? 'opacity-100 translate-x-0' : 'opacity-0 translate-x-12'}`}
      >
        <svg
